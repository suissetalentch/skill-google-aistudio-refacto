      ...
      <aside className="w-full md:w-[35%] bg-[#1a1f2c] p-8 md:p-12 text-white flex flex-col gap-10">
        ...
        <section aria-label="Core Skills">
          ...
          <div className="flex flex-wrap gap-2">
            {data.skills.map((skill, idx) => (
              <span
                key={idx}
                className="bg-slate-800 text-indigo-200 text-[10px] font-black px-3 py-2 rounded-lg border border-slate-700/50 hover:border-indigo-500 transition-all cursor-default"
              >
                {skill}
              </span>
            ))}
          ...
          <section aria-label="Soft Skills & Extras">
            ...
            <div className="flex flex-wrap gap-2">
              {data.additionalSkills.map((skill, idx) => (
                <span
                  key={idx}
                  className="bg-indigo-900/20 text-slate-300 text-[10px] font-medium px-3 py-2 rounded-lg border border-indigo-500/10"
                >
                  {skill}
                </span>
              ))}
        ...
        <section className="mt-auto pt-6 border-t border-slate-800">
          <p className="text-[10px] text-slate-500 italic text-center leading-relaxed">
            {t('preview.footerProfile')}
            <br />
            {t('preview.footerOptimized')}
          </p>
        </section>
      </aside>
                ...
                <div className="flex justify-between items-baseline flex-wrap gap-y-2 mb-6">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-indigo-600 font-black text-[11px] uppercase tracking-widest">
                        {exp.company}
                      </span>
                      <span className="text-slate-300">&bull;</span>
                      <span className="text-slate-400 text-[10px] font-bold uppercase">
                        {exp.location || 'Grenoble'}
                      </span>
                    </div>
                  </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            {data.education.map((edu, idx) => (
              <div
                key={idx}
                className="bg-slate-50 p-6 rounded-2xl border border-slate-100 hover:border-indigo-200 transition-all group"
              >
