...
with 25 systematic anti-patterns that must be fixed before production use.
...
| 3b | Security: `.env` in `.gitignore` | [docs/06-api-services.md](docs/06-api-services.md) | #25 |
...
If no backend proxy exists: STOP and report. The API key cannot be
removed from the client without a backend proxy.
